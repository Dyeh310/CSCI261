...
/* 
   ...
   Project 6
   Can print a calendar 
   of a chosen year
   CSCI 261, Spring 2018 

Please note that I decided to use a premade file name. At first, I had the user type the name of the file they wanted to create, but I noticed your example did not follow this.
For this reason, I changed the file to be consistently named heyd.txt. I hope that is what you wanted.

Question 1: Yes I understand the implementation of the function.
Question 2: Yes I understand the statement.
Question 3: Although our logic was the same, you implemented the function in a different way. Yes I understand yours.
*/
...
int main()
{
	...
	do
	{
		...
		case '3': // print out the calendar of a given year.
			...
			break;
		case '4': //prints calander to file.
			cout << "Please enter a year" << endl;
			cin >> year;
			print_calendar(year, fileName);
			break;
		case '5': // quit the program.
			cout<<"\n\tThank you for using NMH's calendar program!\n\n"; // you may replace your name here
			break;
...
void menu()
{
	...
	cout << "\n\t* 4. Save calendar to output file                 *"; 
	cout << "\n\t* 5. Quit                                         *";
	cout << "\n\t***************************************************";
}
...
bool print_calendar(int year, string fileName)
{
	...
	if (isValid(year) == false){
		cout << "The year is not valid. Please enter a year greater than 1582" << endl;
		return false;
	}
	...
	// return true
	cout << "Calendary of year " << year << " has to been printed to file: " << fileName << endl;
	return true;
}
...
void print_calendar(int month, int year, ofstream& output)
{
	...
	if (isValid(month, year) == false) {
		cout << "The month is not valid. Please enter a number between 1-12" << endl;
		exit(1);
	}
	output << "\n\t " << monthName(month) << ", " << year << endl;
	output << "\n\t SUN MON TUE WED THU FRI SAT\n\t";
	...
	for (int x = 1; x <= days;  x++){
		output << setw(4) << x;
		if((spaceCount + x) % 7 == 0) // go to new line for a new week
			output << "\n\t";
	}
}
